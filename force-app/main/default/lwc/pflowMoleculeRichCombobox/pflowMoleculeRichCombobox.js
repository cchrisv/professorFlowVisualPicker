...
const VALID_BADGE_VARIANTS = new Set([
    'neutral', 'brand', 'success', 'warning', 'error', 'inverse'
]);
...
function normalizeItem(item, index, badgeVariant, badgeShape) {
    const normalizedBadgeVariant = VALID_BADGE_VARIANTS.has(item?.badgeVariant)
        ? item.badgeVariant
        : (VALID_BADGE_VARIANTS.has(badgeVariant) ? badgeVariant : 'neutral');
    const normalizedBadgeShape = VALID_BADGE_SHAPES.has(badgeShape) ? badgeShape : 'pill';
...
export default class PflowMoleculeRichCombobox extends LightningElement {
    ...
    get selectedItem() {
        ...
        return this.normalizedItems.find((item) => item.value === selectedValue) || null;
    }
    ...
    get displayedItems() {
        ...
        return this.filteredItems.map((item, index) => ({
            ...
            titleTokens: buildTokens(item.label, this.enableSearch ? this._searchTerm : '')
        }));
    }
    ...
    handleKeydown(event) {
        ...
        if (event.key === 'End' && this._open) {
            event.preventDefault();
            this._activeIndex = this.findNextEnabledIndex(this.displayedItems.length - 1, -1);
            this.scrollActiveIntoView();
            return;
        }
    ...
    moveActiveIndex(direction) {
        ...
        const start = this._activeIndex < 0
