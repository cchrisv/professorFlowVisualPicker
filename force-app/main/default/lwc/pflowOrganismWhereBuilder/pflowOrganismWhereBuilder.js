...
export function validateCustomLogic(expr, count) {
    if (!expr || !expr.trim()) return { valid: false, error: 'Custom logic is required.' };
    const s = expr.trim();
    ...
    for (const t of tokens) {
        if (/^\d+$/.test(t)) {
            const n = Number(t);
            if (n < 1 || n > count) {
                return { valid: false, error: `Condition #${t} doesn't exist (only ${count} condition${count === 1 ? '' : 's'}).` };
            }
        } else if (!/^(AND|OR)$/i.test(t)) {
            return { valid: false, error: `Unexpected token "${t}". Use numbers, AND, OR, and parentheses.` };
        }
    }
    ...
    for (let i = 0; i < kinds.length - 1; i++) {
        if (kinds[i] === 'OP' && kinds[i + 1] === 'OP') return { valid: false, error: 'Two operators in a row.' };
        if (kinds[i] === 'N' && kinds[i + 1] === 'N') return { valid: false, error: 'Missing AND / OR between conditions.' };
    }
    if (kinds[0] === 'OP') return { valid: false, error: 'Expression starts with an operator.' };
    if (kinds[kinds.length - 1] === 'OP') return { valid: false, error: 'Expression ends with an operator.' };
...
export function renumberCustomLogic(expr, removedIndex) {
    ...
    result = result.replace(new RegExp(`\\s*(AND|OR)\\s+${escaped}\\b`, 'gi'), '');
    result = result.replace(new RegExp(`\\b${escaped}\\s+(AND|OR)\\s*`, 'gi'), '');
    ...
    result = result.replace(/^\s*(AND|OR)\s+/i, '').replace(/\s+(AND|OR)\s*$/i, '');
    return result.trim();
}
...
export function parseCustomWhereClause(str) {
        ...
        parseOperator() {
            ...
            for (const op of ops) {
                if (upper.startsWith(op)) {
                    ...
                    if (/\w/.test(lastOpChar) && afterPos < s.length && /\w/.test(s[afterPos])) continue;
                    this.pos = afterPos;
    ...
    try {
        ...
        if (!hasParens && onlyAnd) return { conditions: parser.conditions, logicMode: 'ALL', customLogic: '' };
        if (!hasParens && onlyOr) return { conditions: parser.conditions, logicMode: 'ANY', customLogic: '' };
        ...
        while (/^\(.*\)$/.test(clean)) {
            ...
            for (let i = 0; i < clean.length - 1; i++) {
                ...
                if (depth === 0 && i < clean.length - 1) { redundant = false; break; }
            }
            if (!redundant) break;
            clean = clean.slice(1, -1).trim();
        }

        return { conditions: parser.conditions, logicMode: 'CUSTOM', customLogic: clean };
    } catch (e) {
        return null;
    }
}
...
export default class PflowOrganismWhereBuilder extends LightningElement {
    ...
    _initFromValue() {
        ...
        if (custom && custom.conditions.length) {
            ...
            this.customLogicError = custom.logicMode === 'CUSTOM'
                ? validateCustomLogic(this.customLogic, custom.conditions.length).error
                : '';
    ...
    _emitChange() {
        ...
        if (this.logicMode === 'CUSTOM' && this.conditions.length > 1) {
            const validation = validateCustomLogic(this.customLogic, this.conditions.length);
    ...
    get previewSoql() {
        ...
        if (this.logicMode === 'CUSTOM' && this.conditions.length > 1) {
            ...
            return v.valid ? serializeConditionsWithCustomLogic(withTypes, this.customLogic) : '';
        }
