...
function mount(overrides = {}) {
    const el = createElement('c-pflow-atom-visual-pick', { is: PflowAtomVisualPick });
    ...
    if (overrides.selectionMode !== undefined) el.selectionMode = overrides.selectionMode;
    ...
    if (overrides.aspectRatio !== undefined) el.aspectRatio = overrides.aspectRatio;
    if (overrides.badgePosition !== undefined) el.badgePosition = overrides.badgePosition;
    if (overrides.badgeVariant !== undefined) el.badgeVariant = overrides.badgeVariant;
...
describe('c-pflow-atom-visual-pick', () => {
    ...
    it('dispatches cardselect on change with value and id', () => {
        ...
        expect(handler.mock.calls[0][0].detail).toEqual({ value: 'first', id: 'i-1' });
    });
    ...
    it('defaults badge to inline + neutral + pill', () => {
        const el = mount();
        const badge = el.shadowRoot.querySelector('.pflow-vpick__badge');
        expect(badge.classList.contains('pflow-vpick__badge_pos-bottom-inline')).toBe(true);
        expect(badge.classList.contains('pflow-vpick__badge_variant-neutral')).toBe(true);
        expect(badge.classList.contains('pflow-vpick__badge_shape-pill')).toBe(true);
    });
    ...
    it('applies badge position/variant/shape modifier classes', () => {
        ...
        const badge = el.shadowRoot.querySelector('.pflow-vpick__badge');
        expect(badge.classList.contains('pflow-vpick__badge_pos-top-right')).toBe(true);
        expect(badge.classList.contains('pflow-vpick__badge_variant-success')).toBe(true);
        expect(badge.classList.contains('pflow-vpick__badge_shape-square')).toBe(true);
    });
    ...
    it('lets item.badgeVariant override the global variant', () => {
        ...
        const badge = el.shadowRoot.querySelector('.pflow-vpick__badge');
        expect(badge.classList.contains('pflow-vpick__badge_variant-warning')).toBe(true);
        expect(badge.classList.contains('pflow-vpick__badge_variant-neutral')).toBe(false);
    });
});
