...
export default class PflowOrganismResourcePicker extends LightningElement {
 ...
 // (e.g. `<c-picker builder-context-filter-collection-boolean>`) arrive
 ...
 const filterColBool = filterColRaw === '' || filterColRaw === 'true' || filterColRaw === true;

 if (this.builderContextFilterType) {
 localOptions = localOptions.filter(opToFilter => opToFilter.displayType?.toLowerCase() === this.builderContextFilterType.toLowerCase() || opToFilter.storeOutputAutomatically === true || ( opToFilter.type.toLowerCase() === 'SObject'.toLowerCase() && !filterColBool));
 }
